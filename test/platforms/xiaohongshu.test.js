...
import * as xiaohongshu from '../../src/platforms/xiaohongshu.js';
...
test('parsePublicPageHtml reads Open Graph video', () => {
  ...
  const result = xiaohongshu.parsePublicPageHtml(html, 'https://www.xiaohongshu.com/explore/test');
...
test('parsePublicPageHtml extracts note data from initial state', () => {
  ...
  const result = xiaohongshu.parsePublicPageHtml(html, 'https://www.xiaohongshu.com/explore/abc123');
...
test('parsePublicPageHtml handles undefined in initial state', () => {
  ...
  const result = xiaohongshu.parsePublicPageHtml(html, 'https://www.xiaohongshu.com/explore/id1');
...
test('detect returns true for share hosts', () => {
  assert.equal(xiaohongshu.detect('https://xhslink.com/o/abc123'), true);
  assert.equal(xiaohongshu.detect('https://www.xiaohongshu.com/explore/abc123'), true);
  assert.equal(xiaohongshu.detect('https://m.xiaohongshu.com/explore/abc123'), true);
});

test('detect returns true for media hosts', () => {
  assert.equal(xiaohongshu.detect('https://sns-video-hw.xhscdn.com/stream/test.mp4'), true);
  assert.equal(xiaohongshu.detect('https://sns-webpic-qc.xhscdn.com/test.jpg'), true);
});

test('detect returns false for unsupported URLs', () => {
  assert.equal(xiaohongshu.detect('https://www.google.com'), false);
  assert.equal(xiaohongshu.detect('not-a-url'), false);
});

test('isMediaHost checks xhscdn.com subdomains', () => {
  assert.equal(xiaohongshu.isMediaHost('xhscdn.com'), true);
  assert.equal(xiaohongshu.isMediaHost('sns-video-hw.xhscdn.com'), true);
  assert.equal(xiaohongshu.isMediaHost('sns-webpic-qc.xhscdn.com'), true);
  assert.equal(xiaohongshu.isMediaHost('example.com'), false);
  assert.equal(xiaohongshu.isMediaHost(''), false);
});

test('name and hosts exports are correct', () => {
  assert.equal(xiaohongshu.name, 'xiaohongshu');
  assert.ok(xiaohongshu.hosts instanceof Set);
  assert.ok(xiaohongshu.hosts.has('xhslink.com'));
  assert.ok(xiaohongshu.hosts.has('www.xiaohongshu.com'));
  assert.ok(xiaohongshu.hosts.has('m.xiaohongshu.com'));
});
