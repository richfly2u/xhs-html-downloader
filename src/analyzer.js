...
function extractJsonObject(text) {
  const source = cleanText(text, 50_000)
    .replace(/^```(?:json)?\s*/i, '')
    .replace(/\s*```$/i, '')
    .trim();
  try {
...
function normalizeAnalysis(value, fallbackSource = '') {
  const fallback = localAnalysis({ title: '', description: fallbackSource, transcript: '' });
  const strings = (items) => Array.isArray(items)
    ? items.map((item) => cleanText(item, 300)).filter(Boolean).slice(0, 5)
    : [];
  const keywords = Array.isArray(value?.keywords)
    ? value.keywords.map((item) => cleanText(item, 60).replace(/^#/, '')).filter(Boolean).slice(0, 10)
    : fallback.keywords;

  return {
    summary: cleanText(value?.summary, 500) || fallback.summary,
    hook: cleanText(value?.hook, 300) || fallback.hook,
    audience: cleanText(value?.audience, 300) || fallback.audience,
    structure: cleanText(value?.structure, 600) || fallback.structure,
    strengths: strings(value?.strengths).length ? strings(value.strengths) : fallback.strengths,
    improvements: strings(value?.improvements).length ? strings(value.improvements) : fallback.improvements,
    keywords,
    optimizedCopy: cleanText(value?.optimizedCopy, 12_000) || fallback.optimizedCopy
  };
}

function buildAnalysisInput({ title, description, transcript }) {
  return [
    title ? `標題：${title}` : '',
    description ? `原始貼文文案：\n${description}` : '',
    transcript ? `影片語音逐字稿：\n${transcript}` : ''
...
async function groqJsonAnalysis({ title, description, transcript }) {
  ...
  const input = buildAnalysisInput({ title, description, transcript });
  ...
  if (!response.ok) {
    ...
    throw new Error(`Groq 文案分析失敗：${detail}`);
  }
  const text = payload?.choices?.[0]?.message?.content;
  if (!text) throw new Error('Groq 沒有回傳可用的分析內容');
  return { analysis: normalizeAnalysis(extractJsonObject(text), input), model, provider: 'groq' };
}
...
async function openAIJsonAnalysis({ title, description, transcript }) {
  ...
  if (!response.ok) {
    const detail = payload?.error?.message || `HTTP ${response.status}`;
    throw new Error(`OpenAI 文案分析失敗：${detail}`);
  }
  const text = getResponseText(payload);
  if (!text) throw new Error('OpenAI 沒有回傳可用的分析內容');
  return { analysis: normalizeAnalysis(extractJsonObject(text), input), model, provider: 'openai' };
}

async function analyzeWithAI(provider, source) {
  if (provider === 'groq') return groqJsonAnalysis(source);
  if (provider === 'openai') return openAIJsonAnalysis(source);
  throw new Error('未設定 AI 金鑰');
}
...
async function groqTranscribeByUrl(videoUrl) {
  const apiKey = process.env.GROQ_API_KEY;
  const parsed = assertHttpUrl(videoUrl);
  if (!isMediaHost(parsed.hostname)) throw new Error('僅允許分析小紅書 CDN 影片');
  await assertPublicResolution(parsed.hostname);
  ...
  form.append('language', process.env.TRANSCRIBE_LANGUAGE || 'zh');
  ...
  if (!response.ok) {
    const detail = payload?.error?.message || `HTTP ${response.status}`;
    throw new Error(`Groq 影片語音辨識失敗：${detail}`);
  }
  return cleanText(payload?.text, 24_000) || null;
}

async function transcribeUploadedVideo(provider, videoUrl) {
  ...
  if (probe.bytes && probe.bytes > maxBytes) {
    throw new Error(`影片 ${(probe.bytes / 1024 / 1024).toFixed(1)} MB，超過語音辨識上限`);
  }
...
async function transcribeVideo(provider, videoUrl) {
  if (!provider || !videoUrl) return { transcript: null, status: '未啟用影片語音辨識' };
  ...
  if (provider === 'groq') {
    try {
      transcript = await groqTranscribeByUrl(videoUrl);
    } catch (urlError) {
      try {
        transcript = await transcribeUploadedVideo(provider, videoUrl);
      } catch (uploadError) {
        const first = urlError instanceof Error ? urlError.message : 'URL 轉錄失敗';
        const second = uploadError instanceof Error ? uploadError.message : '上傳轉錄失敗';
        throw new Error(`${first}；備援上傳亦失敗：${second}`);
      }
    }
  } else {
    transcript = await transcribeUploadedVideo(provider, videoUrl);
  ...
  return {
    transcript,
    status: transcript ? '已完成影片語音轉文字' : '影片沒有辨識到清楚語音'
  };
}
...
export async function analyzeCopy(payload = {}) {
  ...
  const aiConfigured = Boolean(provider);
  ...
  if (aiConfigured && videoUrl) {
    try {
      const transcription = await transcribeVideo(provider, videoUrl);
  ...
  if (!hasSourceText) {
    const error = new Error(aiConfigured
      ? '沒有可分析的標題、貼文文案或影片語音'
      : '這是媒體直連，沒有附帶文案；請先在 Vercel 設定 GROQ_API_KEY，才能聽取影片語音並產生優化稿');
  ...
  if (aiConfigured) {
    try {
      const ai = await analyzeWithAI(provider, { title, description, transcript });
      return {
        ...ai.analysis,
