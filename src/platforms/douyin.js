...
export function extractVideoId(url) {
  try {
    ...
    return match?.[1] || parsed.searchParams.get('vid') || null;
  } catch { return null; }
}
...
function parseInitialState(html) {
  const patterns = [
    ...
    /window\._SSR_HYDRATED_DATA\s*=\s*({.+?});/s,
  ];
...
async function expandAndFetchPage(rawUrl, options) {
  ...
  if (!response.ok) throw new Error(`抖音頁面回應錯誤：HTTP ${response.status}`);
...
export async function resolveShare(inputText, options) {
  ...
  if (isMediaHost(input.hostname)) {
    ...
    return {
      ...
      parser: 'direct-media-url', platform: 'douyin'
    };
  }
  ...
  if (!videoUrl && !images.length) {
    const error = new Error('抖音公開頁面中沒有找到可下載媒體');
